function DI = DI_membrane_EK(V,q,p)
...
dJ_fi_du = p.g_fi_tm*h.*( dm_inf_du.*(u-1.3) + m_inf )/p.tau_fi;
...
dJ_fi_dh = p.g_fi_tm*m_inf.*(u-1.3)/p.tau_fi;
dJ_si_df = d_inf.*(u-1.4)/p.tau_si;

dJ_dh = dJ_fi_dh;
dJ_df = dJ_si_df;
DJ = [dJ_dV,dJ_dh,dJ_df];
DI = (2*pi*p.r*p.Dx)*DJ * (p.V_fi-p.V_rest) * p.cm; 
